...
def home_view(request):
    # Filter published jobs and jobs that are not closed
    published_jobs = Job.objects.filter(is_published=True).order_by('-timestamp')
    jobs = published_jobs.filter(is_closed=False)

    # Get total counts for candidates and companies
    ...
    if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        ...
        # Convert the job objects into a list of dictionaries
        ...
        # Return JSON response with job data and pagination info
        return JsonResponse(data)

    # For regular requests, render the full page with context data
    context = {
